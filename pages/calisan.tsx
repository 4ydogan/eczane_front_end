...
const UrunList = () => {
  ...
  const saveItem = () => {
    ...
    if (item.tc_no) {
      let _items = [...items];
      let _item = { ...item };
      if (_items.filter(val => val.tc_no === item.tc_no).length > 0) {
    ...
    <div className="datatable-crud-demo">
      ...
      <div className="card">
        ...
        <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value)}
          ...
          <Column field="tc_no" header="TC No" sortable style={{ minWidth: '16rem' }}></Column>
          <Column field="ad" header="Ad" sortable style={{ minWidth: '16rem' }}></Column>
          <Column field="soyad" header="Soyad" sortable style={{ minWidth: '16rem' }}></Column>
          <Column field="adres" header="Adres" sortable style={{ minWidth: '16rem' }}></Column>
      ...
      <Dialog visible={itemDialog} style={{ width: '450px' }} header="Item Details" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog}>
        <div className="field">
          ...
          <InputText id="tc_no" value={item.tc_no} autoFocus onChange={(e) => onInputChange(e, 'tc_no')} required className={classNames({ 'p-invalid': submitted && !item.name })} />
          {submitted && !item.tc_no && <small className="p-error">TC No is required.</small>}
        </div>
        <div className="field">
          <label htmlFor="ad">Ad</label>
          <InputText id="ad" value={item.ad} onChange={(e) => onInputChange(e, 'ad')} required className={classNames({ 'p-invalid': submitted && !item.ad })} />
          {submitted && !item.ad && <small className="p-error">Ad is required.</small>}
        </div>
        <div className="field">
          <label htmlFor="soyad">Soyad</label>
          <InputText id="soyad" value={item.soyad} onChange={(e) => onInputChange(e, 'soyad')} required className={classNames({ 'p-invalid': submitted && !item.soyad })} />
          {submitted && !item.soyad && <small className="p-error">Soyad is required.</small>}
        </div>
        <div className="field">
          <label htmlFor="adres">Adres</label>
          <InputTextarea id="adres" value={item.adres} onChange={(e) => onInputChange(e, 'adres')} required rows={3} cols={20} />
        </div>
