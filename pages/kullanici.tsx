    ...
    <div className="datatable-crud-demo">
      ...
      <div className="card">
        ...
        <DataTable ref={dt} value={items} selection={selectedItems} onSelectionChange={(e) => setSelectedItems(e.value)}
          ...
          <Column field="user_id" header="User ID" sortable style={{ minWidth: '16rem' }}></Column>
          <Column field="kullanici_adi" header="Kullanıcı Adı" sortable style={{ minWidth: '16rem' }}></Column>
          <Column field="parola" header="Parola" sortable style={{ minWidth: '16rem' }}></Column>
          <Column field="yetki" header="Yetki" sortable style={{ minWidth: '16rem' }}></Column>
      ...
      <Dialog visible={itemDialog} style={{ width: '450px' }} header="Item Details" modal className="p-fluid" footer={itemDialogFooter} onHide={hideDialog}>
        <div className="field">
          ...
          <InputText id="user_id" value={item.user_id} autoFocus onChange={(e) => onInputChange(e, 'user_id')} required className={classNames({ 'p-invalid': submitted && !item.user_id })} />
          {submitted && !item.user_id && <small className="p-error">User ID is required.</small>}
        </div>
        <div className="field">
          <label htmlFor="kullanici_adi">Kullanıcı Adı</label>
          <InputText id="kullanici_adi" value={item.kullanici_adi} onChange={(e) => onInputChange(e, 'kullanici_adi')} required className={classNames({ 'p-invalid': submitted && !item.kullanici_adi })} />
          {submitted && !item.kullanici_adi && <small className="p-error">Kullanıcı Adı is required.</small>}
        </div>
        <div className="field">
          <label htmlFor="parola">Parola</label>
          <InputText id="parola" value={item.parola} onChange={(e) => onInputChange(e, 'parola')} required className={classNames({ 'p-invalid': submitted && !item.parola })} />
          {submitted && !item.parola && <small className="p-error">Soyad is required.</small>}
        </div>
        <div className="field">
          <label htmlFor="yetki">Yetki</label>
          <Dropdown id="yetki" options={hours} value={item.yetki} virtualScrollerOptions={{ itemSize: 38 }} field="label" dropdown onChange={(e) => onInputChange(e, 'yetki')} />
        </div>
